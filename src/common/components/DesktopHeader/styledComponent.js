...
const HeaderContainer = styled.div `${tw `flex justify-between `}width:1440px;height:80px;border:1px solid #d7dfe9`;
const CompanyLogoAndAppName = styled.div `${tw `flex items-center pl-8`}`;
// const AppName = styled.p `${tw `pl-8`}`;
// const AppName = styled(Typo24DarkBlueGreyHKGroteskMedium);
const UserNameAndProfileImage = styled.div `${tw `flex items-center justify-between pr-8`}`;
